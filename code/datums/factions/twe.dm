...
/datum/faction/twe/modify_hud_holder(image/holder, mob/living/carbon/human/H)
	...
	switch(_role)
		if(JOB_TWE_TML)
			hud_icon_state = "sd" // PLACEHOLDER PLEASE REPLACE
		if(JOB_TWE_MEDIC)
			hud_icon_state = "ld"// PLACEHOLDER PLEASE REPLACE
		if(JOB_TWE_POINTMAN)
			hud_icon_state = "td"// PLACEHOLDER PLEASE REPLACE
		if(JOB_TWE_MARKSMAN)
			hud_icon_state = "ct"// PLACEHOLDER PLEASE REPLACE
		if(JOB_TWE_COMMANDO)
			hud_icon_state = "md"// PLACEHOLDER PLEASE REPLACE
	if(hud_icon_state)
		holder.overlays += image('icons/mob/hud/marine_hud.dmi', H, "pmc_[hud_icon_state]") // PMC IS PLACEHOLDER // PLACEHOLDER PLEASE REPLACE

/datum/faction/twe/get_antag_guns_snowflake_equipment()
...
/datum/faction/twe/get_antag_guns_sorted_equipment()
	// PLACEHOLDER PLEASE REPLACE
	return list(
		list("PRIMARY FIREARMS", 0, null, null, null),
		list("M41A/2 Pulse Rifle", 30, /obj/item/weapon/gun/rifle/m41a/elite, null, VENDOR_ITEM_REGULAR),
